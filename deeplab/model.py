...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, track_running_stats=True, momentum=0.1):
        self.inplanes = 64
        ...
        for i in self.bn1.parameters():
            i.requires_grad = False
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=False)  # TODO changed by nhm
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        x = self.layer5(x)
        return F.interpolate(x, input_size, mode='bilinear')
...
def Res_Deeplab(num_classes=21, is_refine=False, track_running_stats=True, momentum=0.1):
    if is_refine:
        ...
        model = ResNet(Bottleneck, [3, 4, 23, 3], num_classes, track_running_stats, momentum)
    return model
